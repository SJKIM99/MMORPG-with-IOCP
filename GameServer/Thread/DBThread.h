...
//
// 큐 하나를 여러 스레드가 나눠 갖는 대신, "레인(lane)"을 kLaneCount개 두고
// 스레드 하나가 레인 하나를 전담한다. 그리고 같은 행(row)을 건드리는 이벤트는
// 항상 같은 레인으로 보낸다(플레이어 아이템은 playerId 기준, 유저 정보는 name 기준).
//
...
// 옛날 상태가 영구히 남는다. 레인을 고정하면 같은 플레이어의 쓰기는 한 스레드가
// 넣은 순서 그대로 하나씩 처리하므로 이 역전이 구조적으로 불가능해진다.
//
// 병렬성은 그대로다 — 스레드도 커넥션도 여전히 kLaneCount개가 전부 돌고, 서로
// 다른 플레이어끼리는 예전처럼 완전히 병렬로 처리된다. playerId가 DB
// auto-increment 정수라 나머지 연산만으로도 레인에 거의 균등하게 흩어진다.
class DBThread
{
public:
	...
	// 레인 수 = DB 워커 스레드 수. GameServer.cpp의 스레드 생성 루프와 반드시
	// 같아야 하므로(레인에 담당 스레드가 없으면 그 레인 이벤트는 영원히 처리되지
	...
	// laneIndex 레인만 전담해서 처리한다. 스레드 하나당 하나씩, 0 ~ kLaneCount-1을
	// 빠짐없이 한 번씩 맡겨야 한다.
	void DoDataBase(int laneIndex);
	...
	// 같은 playerId의 요청은 항상 같은 레인으로 가므로 넣은 순서대로 반영된다.
...
private:
	...
	void ScheduleOnLane(shared_ptr<DB_EVENT_BASE> event, int laneIndex);
	void ProcessEvent(const shared_ptr<DB_EVENT_BASE>& event);

	// 같은 행을 건드리는 이벤트가 항상 같은 레인으로 가게 하는 매핑.
	// Players 테이블(name 키)과 Inventory 테이블(playerId 키)은 서로 다른 행이라
	// 같은 레인에 모일 필요가 없다 — 각자 자기 키 기준으로만 일관되면 된다.
	[[nodiscard]] static int LaneForPlayerId(int playerId) noexcept;
	[[nodiscard]] static int LaneForName(const std::string& name) noexcept;
	// 순서를 맞출 대상이 없는 이벤트(로그인: 아직 playerId를 모르는 조회 작업)용.
	// 아무 레인에나 가도 되므로 부하만 고르게 흩어준다.
	[[nodiscard]] int NextRoundRobinLane() noexcept;

private:
	std::array<Lane, kLaneCount> _lanes;
	// 레인별 락과 무관하게 전역으로 증가한다 — 같은 wakeupTime을 가진 이벤트끼리
