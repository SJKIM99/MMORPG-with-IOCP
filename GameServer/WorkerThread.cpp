...
void WorkerThread::Disconnect(ObjID clientId)
{
	...
	GSector->ForEachNeighborObject(user->GetSectorX(), user->GetSectorY(), [&](const shared_ptr<Subject>& subject)
	{
		...
		if (session == nullptr || session->_state != SOCKET_STATE::ST_INGAME) return;
		if (!WorldHelper::CanSee(subjectId, clientId)) return;
	...
	while (true)
	{
		...
		case IO_TYPE::IO_ACCEPT:
		{
			...
			if (session != nullptr)
			{
				...
				session->_state = SOCKET_STATE::ST_ALLOC;
		...
		case IO_TYPE::IO_RECV:
		{
			...
			uint32 remainData = numOfBytes + session->_pendingRecvBytes;
			...
			session->_pendingRecvBytes = remainData;
			if (remainData > 0)
